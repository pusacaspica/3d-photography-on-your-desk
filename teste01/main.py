...
time = shadowTime(imgs, 120, 240, thresh)
print(time)

paintX(imgs[time], 120)
paintY(imgs[time], 240)
...
paintX(deltas[31], 120)
paintY(deltas[31], 240)
...
plotPoints = np.ndarray((imgs[0].shape[0], imgs[0].shape[1]), dtype=np.uint8)
print(str(plotPoints.shape))
for i in range(imgs[0].shape[0]):
    for j in range(imgs[0].shape[1]):
        plotPoints = shadowTime(imgs, j, i, thresh)
print(plotPoints)
